...
void VectorConvectionNLFIntegrator::AssembleMFBoundary(const FiniteElementSpace
                                                       &fes)
{
   MFEM_ASSERT(fes.GetOrdering() == Ordering::byNODES,
               "MF only supports Ordering::byNODES!");
   ...
   {
      delete ceedOp;
      ceedOp = new ceed::MFVectorConvectionNLIntegrator(*this, fes, Q, true);
      return;
   }
   ...
   // const IntegrationRule *ir = IntRule ? IntRule : &GetRule(el, T);
   MFEM_ABORT("Error: VectorConvectionNLFIntegrator::AssembleMFBoundary only"
              " implemented with libCEED");
}
